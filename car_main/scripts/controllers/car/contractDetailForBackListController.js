 ...
 */
'use strict';
angular.module('sbAdminApp').controller('ContractDetailForBackListCtrl', ['$rootScope', '$scope', 'Init', 'Modal', 'localStorageService', '$timeout', '$interval', 'CheckBrowser','$state','$stateParams', function ($rootScope, $scope, Init, Modal, localStorageService, $timeout, $interval, CheckBrowser,$state,$stateParams) {
    CheckBrowser.check();
    $scope.func_class = {
        "display":"flex",
    ...
    $scope.init = function(){
        Init.iwbhttp('/car/getContract', par, function (data, header, config, status) {
            if(data.resFlag == '0'){
                ...
                if(data.contractBack && data.contractBack != null){
                    $scope.contractBack = data.contractBack;
                }
                ...
                if(data.financeFlowList && data.financeFlowList != null){
                    ...
                    for(var i = 0 ; i < $scope.financeFlowList.length ; i++){
                        if($scope.financeFlowList[i].BIZ_NAME == '合同退款'){
                            removeIndex = i
    ...
    $scope.collectSub = function(obj){
        $scope.contractBack.ID = id
        var par = {}
        par.contractBack  = $scope.contractBack
        par.contractBack.type = obj
        var msg = "请确认后提交?";
        ...
        var modalInstance = Modal.modal(url, ctrlName, resolve, function (returnData) {
            Init.iwbhttp('/car/saveContractBack', returnData, function (data, header, config, status) {
                if(data.resFlag == '0'){
                    $scope.open(data.msg)
    ...
    $scope.financeSub = function(){
        ...
        var modalInstance = Modal.modal(url, ctrlName, resolve, function (returnData) {
            Init.iwbhttp('/car/saveFinanceFlow', returnData, function (data, header, config, status) {
                if(data.resFlag == '0'){
    ...
    $scope.updateContract = function() {
        Init.iwbhttp("/car/updateContractForBack", {id:id}, function (data, header, config, status) {
			if(data.resFlag == '0'){
                $scope.contract.STATUS = '5'
                $scope.open(data.msg)
			}else{
                $scope.open(data.msg)
            }     
    ...
    $scope.openCarTableBack = function (){
        ...
        ctrlName = 'CarTableBackModalCtrl';
        var parm = {
            id:$scope.contract.ID,
        };
        resolve = {
