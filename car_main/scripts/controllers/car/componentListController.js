'use strict';
angular.module('sbAdminApp').controller('ComponentListCtrl', ['$scope','Init','Modal','$state','CheckBrowser','CheckParam', function ($scope,Init,Modal,$state,CheckBrowser,CheckParam) {
    CheckBrowser.check();
    ...
    $scope.pageData = "";
    $scope.param = {};
    var table = $('#componetTable').DataTable({
        "serverSide": true,
        "columns": [
            {
                "visible": false,
        ...
        ajax:function(data, callback, settings){
            ...
            Init.iwbhttp('/car/componentList', $scope.param, function(data,header,config,status){
                var returnData = {};
                if(data.resFlag == 0){
                    returnData.recordsTotal = data.totalRow;//返回数据全部记录
                    returnData.recordsFiltered = data.totalRow;//后台不实现过滤功能，每次查询均视作全部结果
                    returnData.data = data.componentList;//返回的数据列表
                    callback(returnData);
                }else{
                    $scope.open(data.msg);
                }
