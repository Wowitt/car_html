'use strict';
angular.module('sbAdminApp').controller('CarTableReplaceModalCtrl', ['$scope','$modalInstance','Init','content','parm','Modal', function ($scope, $modalInstance,Init,content,parm,Modal){
	//弹框标题
	...
	var ctrlName = "";
	//查询申请的流程列表
	Init.iwbhttp('/car/queryCarTableReplace', {id:$scope.parm.id}, function (data, header, config, status) {
	...
	$scope.save = function(){
		...
		Init.iwbhttp('/car/saveCarTableReplace', {list:$scope.dataList}, function (data, header, config, status) {
			$scope.open(data.msg);
			for(var i = 0 ;i<$scope.dataList.length; i++){
				$scope.dataList[i].DRIVING_LICENSE = $scope.dataList[i].DRIVING_LICENSE == '1' ? true : false;
				$scope.dataList[i].MAINTENANCE_SIGN = $scope.dataList[i].MAINTENANCE_SIGN =='1' ? true : false;
